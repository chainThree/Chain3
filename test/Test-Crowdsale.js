var Chain3Crowdsale = artifacts.require("Chain3Crowdsale");
var Chain3Token = artifacts.require("Chain3Token");

contract('Chain3Crowdsale', function(accounts) {
    it('should deploy the token and store the address', function(done){
        Chain3Crowdsale.deployed().then(async function(instance) {
    ...
    it('one ETH should buy 5 Chain3 Tokens in PreICO', function(done){
        Chain3Crowdsale.deployed().then(async function(instance) {
            const data = await instance.sendTransaction({ from: accounts[7], value: web3.toWei(1, "ether")});
            const tokenAddress = await instance.token.call();
            const Chain3Token = Chain3Token.at(tokenAddress);
            const tokenAmount = await Chain3Token.balanceOf(accounts[7]);
    ...
    it('one ETH should buy 2 Chain3 Tokens in ICO', function(done){
        Chain3Crowdsale.deployed().then(async function(instance) {
            const data = await instance.sendTransaction({ from: accounts[2], value: web3.toWei(1.5, "ether")});
            const tokenAddress = await instance.token.call();
            const Chain3Token = Chain3Token.at(tokenAddress);
            const tokenAmount = await Chain3Token.balanceOf(accounts[2]);
